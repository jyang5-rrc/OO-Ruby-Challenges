...
class BobTest < Minitest::Test
  def bob # Instead of calling ::Bob.new in every test, you call bob to get a new instance.
    ::Bob.new
  end
  ...
  def test_prolonged_silence
    #skip
    remark = ' ' * rand(1..10) # rand(1..10) returns a random number between 1 and 10
    assert_equal 'Fine. Be that way!', bob.hey(remark), feedback(remark)
  end

  def test_alternate_silences
    #skip
    remark = "\t" * rand(1..10)# "\t" is the escape sequence for a tab
    assert_equal 'Fine. Be that way!', bob.hey(remark), feedback(remark)
  end
